...
import 'edit_checkup_model.dart';
export 'edit_checkup_model.dart';

class EditCheckupWidget extends StatefulWidget {
  const EditCheckupWidget({
    super.key,
    required this.checkupId,
  });

  final DailyCheckupRecord? checkupId;

  @override
  State<EditCheckupWidget> createState() => _EditCheckupWidgetState();
}

class _EditCheckupWidgetState extends State<EditCheckupWidget>
    with TickerProviderStateMixin {
  late EditCheckupModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EditCheckupModel());

    _model.souvenirTextController ??=
        TextEditingController(text: widget.checkupId?.messageDuJour);
    _model.souvenirFocusNode ??= FocusNode();

    _model.conseilTextController ??=
        TextEditingController(text: widget.checkupId?.conseilDuJour);
                    ...
                    child: FFButtonWidget(
                      onPressed: () async {
                        if (_model.compteur < 2) {
                          ...
                          await widget.checkupId!.reference
                              .update(createDailyCheckupRecordData(
                            messageDuJour: _model.souvenirTextController.text,
                            humeurDuJour: _model.choiceChipsValue,
                            conseilDuJour: _model.conseilTextController.text,
                            humeurNumber: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Votre checkup a bien été modifié!',
